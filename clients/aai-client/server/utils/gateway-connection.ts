...
export async function getGateway(user: string): Promise <Gateway> {
    ...
    const keyDirectoryPath = envOrDefault('KEY_DIRECTORY_PATH', path.resolve(cryptoPath, 'users', `${user}@org3.example.com`, 'msp', 'keystore'));
    // Path to user certificate.
    const certPath = envOrDefault('CERT_PATH', path.resolve(cryptoPath, 'users', `${user}@org3.example.com`, 'msp', 'signcerts', 'cert.pem'));
