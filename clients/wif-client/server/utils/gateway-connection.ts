...
export async function getGateway(user: string): Promise <Gateway> {
    ...
    const cryptoPath = envOrDefault('CRYPTO_PATH', path.resolve('test-network', 'organizations', 'peerOrganizations', 'org2.example.com'));

    // Path to user private key directory.
    const keyDirectoryPath = envOrDefault('KEY_DIRECTORY_PATH', path.resolve(cryptoPath, 'users', `${user}@org2.example.com`, 'msp', 'keystore'));
    // Path to user certificate.
    const certPath = envOrDefault('CERT_PATH', path.resolve(cryptoPath, 'users', `${user}@org2.example.com`, 'msp', 'signcerts', 'cert.pem'));
